    ...
    public synchronized void write(BleDevice bleDevice,
        ...
        if (bleBluetooth == null) {
            ...
            if (split && data.length > 1) {
                new SplitWriter().splitWrite(bleBluetooth, uuid_service, uuid_write,  data, iDataEngine, callback);
            } else {
                bleBluetooth.newBleConnector()
                        .withUUIDString(uuid_service, uuid_write)
                        .writeCharacteristic(iDataEngine.packetData(data), callback, uuid_write);
            }
        }
    }
