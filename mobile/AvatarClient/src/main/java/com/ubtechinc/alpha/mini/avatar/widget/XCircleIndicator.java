...
public class XCircleIndicator extends View {
    ...
    public void setCount(int count, int currentPage) {
        this.count = count;
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        Log.d("onDraw==", "width==" + getWidth() + "  height==" + getHeight());
        // Draw stroked circles
        if (count <= upLimit) {
            for (int iLoop = 0; iLoop < count; iLoop++) {
                canvas.drawCircle(getPaddingLeft() + radius
                                + (2*iLoop * (2 * radius + circleInterval)),
                        getPaddingTop() + radius, radius, mPaintStroke);
                Log.d("onDraw==", "x==" + getPaddingLeft() + radius
                        + (iLoop * (2 * radius + circleInterval)) + "  y==" + getPaddingTop() + radius);
            }
            int cx = 0;
            // if (flowWidth != 0) {
            ...
            // }
            cx = 2*(2 * radius + circleInterval) * currentPage;
    ...
    private int measureWidth(int measureSpec) {
        ...
        if (specMode == MeasureSpec.EXACTLY) {
            ...
            result = getPaddingLeft() + getPaddingRight()
                    + 2*(count * 2 * radius) + 2*(count - 1) * circleInterval;
